using DemoLibrary.ConcreteComponents;

namespace DemoLibrary.VisitorInterfaces;

/// <summary>
/// The Visitor Interface declares a set of visiting methods that correspond
/// to component classes. The signature of a visiting method allows the
/// visitor to identify the exact class of the component that it's dealing
/// with.
/// </summary>
public interface IHealthcareVisitor
{
    void VisitPediatricPatient(PediatricPatient patient);
    void VisitAdultPatient(AdultPatient patient);
    void VisitElderlyPatient(ElderlyPatient patient);
}

